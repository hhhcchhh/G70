...
public class GpsOffsetTestDialog extends Dialog {
	...
	public GpsOffsetTestDialog(Context context, DwDeviceInfoBean dwDevice) {
    	super(context, R.style.style_dialog);
        this.mContext = context;
        this.device = dwDevice;
    ...
    private void initView(View view) {
		...
		btn_ok.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View view) {
				...
				if (device.getWorkState() != GnbBean.DW_State.IDLE) {
					MainActivity.getInstance().showToast(mContext.getString(R.string.dev_busy_tip));
					return;
				}
				if (device.getRsp().getGpsSyncState() != GnbStateRsp.Gps.SUCC) {
					MainActivity.getInstance().showToast(mContext.getString(R.string.gps_unlock));
					return;
				}
	...
	private void startTd(){
		...
		if (isNotStart){
			index = -1;
			MainActivity.getInstance().showToast(mContext.getString(R.string.test_finish));
	...
	private void startTdRun(ArfcnBean bean){
		...
		PaCtl.build().openPA(device.getRsp().getDeviceId(), String.valueOf(arfcn));
		new Handler().postDelayed(new Runnable() {
			...
			public void run() {
				...
				MessageController.build().startDWTdMeasure(device.getRsp().getDeviceId(), cell_id, 0, arfcn, 0, 0);
			}
		}, 200);
	}
	...
	private void addToCurCity() {
		...
		if (hasOk) {
			...
			MainActivity.getInstance().showToast(mContext.getString(R.string.no_data));
		}
	}
	...
	private final DwDeviceInfoBean device;
