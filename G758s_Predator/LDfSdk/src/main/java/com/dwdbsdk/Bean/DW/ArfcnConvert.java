...
public class ArfcnConvert {
    ...
    public static int parse5to4(String arfcn) {
        ...
        float FREF_Offs;
        float NREF_Offs;
        int ARFCN = Integer.parseInt(arfcn);//633984;629952[42082-3];633984[42688]
        asb.append("[ ARFCN: ").append(ARFCN);
        asb.append(",");
        ...
        asb.append("ΔFGloba: ").append(ΔFGloba);
        asb.append(",");
        asb.append("FREF_Offs: ").append(FREF_Offs);
        asb.append(",");
        asb.append("NREF_Offs: ").append(NREF_Offs);
        asb.append(",");
        float f5g = FREF_Offs + ΔFGloba * (ARFCN - NREF_Offs);
        asb.append("NR[C-FRE]: ").append(f5g);
        asb.append(",");
        ...
        asb.append("ifdl_low: ").append(ifdl_low);
        asb.append(",");
        asb.append("inoffs_dl: ").append(inoffs_dl);
        asb.append(",");

        float ndl = (f5g - ifdl_low) * 10 + inoffs_dl;
        asb.append("ARFCN-L: ").append(ndl);
        asb.append(",");
        ...
        asb.append("(").append(indl);
        asb.append(")]");
        SdkLog.D("ArfcnConvert: " + asb);
        return indl;
    }
    ...
    private static float calaMidFrq(int arfcn) {
        ...
        return FREF_Offs + ΔFGloba * (arfcn - NREF_Offs);
    }

    /**
     * 根据中心频点算GSCN
     *
     * @param f5g  中心频点,所有数据单位为: kHz
     * @param type 频点类型
     * @return gscn
     */
