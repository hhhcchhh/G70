...
public class GpsOffsetTestDialog extends Dialog {
    ...
    private void initView(View view) {
        ...
        btn_ok.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (MainActivity.getInstance().getDeviceList().get(0).getWorkState() != GnbBean.DW_State.IDLE) {
    ...
    private void startTdRun(ArfcnBean bean) {
        PaCtl.build().closePAById(MainActivity.getInstance().getDeviceList().get(0).getRsp().getDeviceId());
        ...
        int index = DeviceUtil.getIndexByChannelNum(DeviceUtil.getChannelNumByarfcn_3G758_CX(String.valueOf(arfcn)));
        PaCtl.build().openPA(MainActivity.getInstance().getDeviceList().get(index).getRsp().getDeviceId(), String.valueOf(arfcn), DeviceUtil.getChannelNumByarfcn_3G758_CX(String.valueOf(arfcn)));
        new Handler().postDelayed(() -> {
            // startTdMeasure(int cell_id, int swap_rf, int arfcn, int pk, int pa)
            int cell_id = DeviceUtil.getCellIdByChannelNum(DeviceUtil.getChannelNumByarfcn_3G758_CX(String.valueOf(arfcn)));
