...
public class TraceBean {
    ...
    public final static int RSRP_TIME_INTERVAL = 1000; // 报值间隔时间
    ...
    public boolean isArfcnChange() {
        return arfcnChange;
    }

    public void setArfcnChange(boolean arfcnChange) {
        this.arfcnChange = arfcnChange;
    }
...
//        if (lastRsrp == traceRsrp && traceRsrp > 0) {
//            if (++sameRsrpCnt > 5) { // 连续5次报值一样处理
//                sameRsrpCnt = 0;
//                lostCount = 0;
//                traceRsrp = -1; // -1 掉线
//            }
//        }
        APPLog.D("getTraceRsrp: id = " + id + ", lostCount = " + lostCount + ", traceRsrp = " + traceRsrp + ", lastRsrp = " + lastRsrp + ", sameRsrpCnt = " + sameRsrpCnt);
        if (traceRsrp == 0) {
            if (++lostCount >= 8) {
                lostCount = 0;
                sameRsrpCnt = 0;
