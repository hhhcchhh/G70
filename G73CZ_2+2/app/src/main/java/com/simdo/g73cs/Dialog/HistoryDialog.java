...
public class HistoryDialog extends Dialog implements OnClickListener {
    ...
    private void initView() {
        try {
            ...
            for (int i = jsonArray.length() - 9; i > 0; i = i - 9) {
                //通道1至8、时间
                ...
                jsonObjectList5.add(jsonArray.getJSONObject(i + 4));
                jsonObjectList6.add(jsonArray.getJSONObject(i + 5));
                jsonObjectList7.add(jsonArray.getJSONObject(i + 6));
                jsonObjectList8.add(jsonArray.getJSONObject(i + 7));
                currentTimeList.add(jsonArray.getString(i + 8));
            }
        ...
        mListAdapter = new HistoryAdapter(mContext, jsonObjectList1, jsonObjectList2, jsonObjectList3, jsonObjectList4,
                jsonObjectList5, jsonObjectList6, jsonObjectList7, jsonObjectList8, currentTimeList);
        ...
        switch (v.getId()) {
            case R.id.tv_delete:
                ...
                view.findViewById(R.id.btn_ok).setOnClickListener(view1 -> {
                    ...
                    jsonObjectList5.clear();
                    jsonObjectList6.clear();
                    jsonObjectList7.clear();
                    jsonObjectList8.clear();
                    currentTimeList.clear();
                    mListAdapter.setList(jsonObjectList1, jsonObjectList2, jsonObjectList3, jsonObjectList4,
                            jsonObjectList5, jsonObjectList6, jsonObjectList7, jsonObjectList8, currentTimeList);
    ...
    private void showCityConfirmDialog(final int position, final boolean delete) {
        ...
        view.findViewById(R.id.btn_ok).setOnClickListener(v -> {
            ...
            JSONObject jsonObject5 = jsonObjectList5.get(position);
            JSONObject jsonObject6 = jsonObjectList6.get(position);
            JSONObject jsonObject7 = jsonObjectList7.get(position);
            JSONObject jsonObject8 = jsonObjectList8.get(position);
            ...
            if (delete) {
                ...
                jsonObjectList5.remove(position);
                jsonObjectList6.remove(position);
                jsonObjectList7.remove(position);
                jsonObjectList8.remove(position);
    ...
    private final ArrayList<JSONObject> jsonObjectList5 = new ArrayList<>();
    private final ArrayList<JSONObject> jsonObjectList6 = new ArrayList<>();
    private final ArrayList<JSONObject> jsonObjectList7 = new ArrayList<>();
    private final ArrayList<JSONObject> jsonObjectList8 = new ArrayList<>();
