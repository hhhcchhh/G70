...
public class FreqUtil {
     ...
     * */
    public LinkedList<LinkedList<ArfcnPciBean>> decFreqList(LinkedList<ScanArfcnBean> freqList) {
        ...
        if (!listIsContains(TD1, arfcn)) {
            ...
            if (bandNumMap.containsKey(bandStr)) {
                ...
                for (int i = 0; i < listNum.size(); i++) {
                    if (maxRsrp > -80){ // 大于-80，按照扫频强度rsrp来选择，rsrp越大，优先权越高
                        boolean isAdd = false;
                        do {
                        ...
                        break;
                    }else { // 小于等于-80按照扫频PCI数量多少来选择，个数越多，优选权越高，-80之后不做rsrp排序
