...
//        setTabWidthSame(tab_layout);
    ...
    public void doSetTxPwrOffset(final int value) {
        AppLog.I("TraceFragment doSetTxPwrOffset value = " + value + " txValue = " + txValue);
        ...
        for (DeviceInfoBean bean : MainActivity.getInstance().getDeviceList()) {
            ...
            AppLog.I("TraceFragment doSetTxPwrOffset getTraceType = " + MessageController.build().getTraceType(id));
            ...
            mFreqDialog.setOnDismissListener(new DialogInterface.OnDismissListener() {
                ...
                public void onDismiss(DialogInterface dialog) {
                    if (!mFreqDialog.isUserCancel()) {
                        ...
                        int airSync = PrefUtil.build().getValue("sync_mode", "Air").toString().equals("GPS") ? 0 : 1;
                            ...
                            switch (rsp.getCellId()) {
                                case 0:
                                    if (linkedList.size() == 1) {
                                        ...
                                        autoArfcnIndex0++;
                                        if (autoArfcnIndex0 >= linkedList.size()) autoArfcnIndex0 = 0;
                                ...
                                case 1:
                                    if (linkedList.size() == 1) {
                                        ...
                                        autoArfcnIndex1++;
                                        if (autoArfcnIndex1 >= linkedList.size()) autoArfcnIndex1 = 0;
                                ...
                                case 2:
                                    if (linkedList.size() == 1) {
                                        ...
                                        autoArfcnIndex2++;
                                        if (autoArfcnIndex2 >= linkedList.size()) autoArfcnIndex2 = 0;
                                ...
                                case 3:
                                    if (linkedList.size() == 1) {
                                        ...
                                        autoArfcnIndex3++;
                                        if (autoArfcnIndex3 >= linkedList.size()) autoArfcnIndex3 = 0;
        ...
        if (rsp.getCmdRsp() != null) {
            ...
            if (rsp.getCmdRsp().getCmdType() == GnbProtocol.UI_2_gNB_START_TRACE || rsp.getCmdRsp().getCmdType() == GnbProtocol.UI_2_gNB_START_LTE_TRACE) {
                ...
                if (rsp.getCmdRsp().getRspValue() == GnbProtocol.OAM_ACK_OK) {
                    ...
                    MessageController.build().setTxPwrOffset(id, cell_id, Integer.parseInt(traceUtil.getArfcn(cell_id)), mCfgTraceChildFragment.isCloseDistance() ? -2 : 0);
...
//                                mImsiList.get(j).setArfcn(traceArfcn);
                ...
                if (MainActivity.getInstance().getDeviceList().get(indexById).getTraceUtil().getWorkState(cell_id) == GnbBean.State.TRACE && !isRsrpStart && rsp.getRsrp() > 5) {
                    countFirst = 0;
                    countSecond = 0;
                    countThird = 0;
                    countFourth = 0;
    ...
    public void onStopTraceRsp(String id, GnbCmdRsp rsp) {
        ...
        if (rsp != null) {
            if (rsp.getCmdType() == GnbProtocol.UI_2_gNB_STOP_TRACE || rsp.getCmdType() == GnbProtocol.UI_2_gNB_STOP_LTE_TRACE) {
                ...
                switch (rsp.getCellId()) {
                    case 0:
                        if (autoRun1 && MainActivity.getInstance().getDeviceList().get(0).getTraceUtil().getWorkState(GnbProtocol.CellId.FIRST) != GnbBean.State.STOP) {
                            updateParamAndStart(id, arfcn, pci, rsp.getCellId());
                            return;
                        }
                        break;
                    case 1:
                        if (autoRun2 && MainActivity.getInstance().getDeviceList().get(0).getTraceUtil().getWorkState(GnbProtocol.CellId.SECOND) != GnbBean.State.STOP) {
                            updateParamAndStart(id, arfcn, pci, rsp.getCellId());
                            return;
                        }
                        break;
                    case 2:
                        if (autoRun3 && MainActivity.getInstance().getDeviceList().get(0).getTraceUtil().getWorkState(GnbProtocol.CellId.THIRD) != GnbBean.State.STOP) {
                            updateParamAndStart(id, arfcn, pci, rsp.getCellId());
                            return;
                        }
                        break;
                    case 3:
                        if (autoRun4 && MainActivity.getInstance().getDeviceList().get(0).getTraceUtil().getWorkState(GnbProtocol.CellId.FOURTH) != GnbBean.State.STOP) {
                            updateParamAndStart(id, arfcn, pci, rsp.getCellId());
                            return;
                        }
                        break;
...
//                                    updateParam(0, value, mTraceUtilNr.getPci(GnbProtocol.CellId.FIRST));
                                    //更新页面信息
                                    freshDoWorkState(0, GnbProtocol.CellId.FIRST, mTraceUtilNr.getImsi(GnbProtocol.CellId.FIRST), value, mTraceUtilNr.getPci(GnbProtocol.CellId.FIRST));
                                    ...
                                    if (autoArfcnIndex0 >= arfcnList.size()) autoArfcnIndex0 = 0;
                                } else {
...
//                                    updateParam(1, value, mTraceUtilNr.getPci(GnbProtocol.CellId.SECOND));
                                    //更新页面信息
                                    freshDoWorkState(0, GnbProtocol.CellId.SECOND, mTraceUtilNr.getImsi(GnbProtocol.CellId.SECOND), value, mTraceUtilNr.getPci(GnbProtocol.CellId.SECOND));
                                    ...
                                    autoArfcnIndex1++;
                                    if (autoArfcnIndex1 >= arfcnList.size()) autoArfcnIndex1 = 0;
                                } else {
...
//                                    updateParam(2, value, mTraceUtilNr.getPci(GnbProtocol.CellId.THIRD));
                                    //更新页面信息
                                    freshDoWorkState(0, GnbProtocol.CellId.THIRD, mTraceUtilNr.getImsi(GnbProtocol.CellId.THIRD), value, mTraceUtilNr.getPci(GnbProtocol.CellId.THIRD));
                                    ...
                                    autoArfcnIndex2++;
                                    if (autoArfcnIndex2 >= arfcnList.size()) autoArfcnIndex2 = 0;
                                } else {
...
//                                        updateParam(3, value, mTraceUtilNr.getPci(GnbProtocol.CellId.FOURTH));
                                        //更新页面信息
                                        freshDoWorkState(0, GnbProtocol.CellId.FOURTH, mTraceUtilNr.getImsi(GnbProtocol.CellId.FOURTH), value, mTraceUtilNr.getPci(GnbProtocol.CellId.FOURTH));
    ...
    private void freshDoWorkState(int type, int cell_id, String imsi, String arfcn, String pci) {
        ...
        if (!imsi.isEmpty()) {
            ...
            mTraceChildFragment.setConfigInfo(cell_id, type, arfcn, pci, imsi);
        }
    }
...
//        }, 200);
    ...
    //将当前的数据存入历史记录
    private void storeAutoInfoIntoHistory(LinkedList<ArfcnPciBean> td1, LinkedList<ArfcnPciBean> td2, LinkedList<ArfcnPciBean> td3, LinkedList<ArfcnPciBean> td4) {
    ...
    private void startRunWork() {
        ...
        boolean isAirSync = !PrefUtil.build().getValue("sync_mode", "Air").toString().equals("GPS");
        autoArfcnIndex0 = 0;
        autoArfcnIndex1 = 0;
        autoArfcnIndex2 = 0;
        autoArfcnIndex3 = 0;
