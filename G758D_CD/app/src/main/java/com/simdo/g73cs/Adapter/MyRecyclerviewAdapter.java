...
public class MyRecyclerviewAdapter extends RecyclerView.Adapter<MyRecyclerviewAdapter.ViewHolder> {
    ...
    public MyRecyclerviewAdapter(Context context, List<ArfcnPciBean> arfcnList, String band) {
        this.context = context;
        this.arfcnList.clear();
        for (ArfcnPciBean bean : arfcnList) {
            this.arfcnList.add(bean.getArfcn());
        }
        this.band = band;
    }
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        if (position == getItemCount() - 1) {
            if (holder.arfcn.getText().toString().isEmpty()) {
                ...
                holder.arfcn.setFocusable(true);
                holder.arfcn.setFocusableInTouchMode(true);
                ...
                holder.arfcn.postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        InputMethodManager inputManager =
                                (InputMethodManager) holder.arfcn.getContext().getSystemService(Context.INPUT_METHOD_SERVICE);
                        inputManager.showSoftInput(holder.arfcn, InputMethodManager.SHOW_FORCED);
                    }
                }, 200); // 可能需要调整延迟时间以适应您的布局加载速度
...
//        holder.btn_del.setFocusableInTouchMode(holder.ifClickable);
    ...
    public boolean addData(String arfcn) {
        if (!arfcnList.contains(arfcn)) {
            AppLog.I("addData arfcnList.contains(arfcn) arfcn = " + arfcn);
            if (arfcn == null || arfcn.isEmpty()) return true; //为空或为null不做任何处理
            if (getItemCount() > 0) {
    ...
    public boolean addDataToFirst(String arfcn) {
        if (lastEditText != null && lastEditText.getEditableText().length() > 0) {
            addData(lastEditText.getEditableText().toString());
        }
        if (!arfcnList.contains(arfcn)) {
            AppLog.I("addData arfcnList.contains(arfcn) arfcn = " + arfcn);
            if (arfcn == null || arfcn.isEmpty()) return true; //为空或为null不做任何处理
...
//                if ()
    ...
    private boolean storeLastText2(String key) {
        ...
        String text;
        if (lastEditText == null && arfcnList.size() <= 1) {
            return true;
        }
        if (lastEditText == null) {
            //如果已经刷新过，用list倒数第二个数来判断
            text = arfcnList.get(arfcnList.size()-2);
        } else {
            text = lastEditText.getText().toString();
        }
        ...
        AppLog.I("CfgTraceChildFragment arfcnList: " + arfcnList + " text: " + text);

        if (lastEditText == null) { //lastEditText == null且前一个数通过筛选
            return true;
        }
        text = lastEditText.getText().toString();   //复原
