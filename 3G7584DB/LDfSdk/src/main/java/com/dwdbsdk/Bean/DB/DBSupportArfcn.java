...
import com.dwdbsdk.Util.SsbUtil;
...
public class DBSupportArfcn {
    ...
    private void init() {
        ...
        if (list == null || list.equals("")) {
            ...
            //4.9G
            arfcnList.add(new ArfcnBeanSsb(723360, 0, 14, 230, 4850010000L));
            //2.1G freq error
    ...
    public boolean add(int arfcn, int pk, int pa, int timingOffset) {
        ...
        if (add) {
            ...
            int tOffset = 0;
            if (band == 41 || band == 79) {
                tOffset = 3000000 - timingOffset;
            }
            long freqCarrier = SsbUtil.build().OrxFreqCarrier(arfcn, pk, pa);
            arfcnList.add(new ArfcnBeanSsb(arfcn, tOffset, pk, pa, freqCarrier));
            SdkLog.D("add: " + arfcn + ", " + tOffset + ", " + pk + ", " + pa + ", " + freqCarrier);
            saveToJson();
        }
        return add;
    }
