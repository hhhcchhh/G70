...
//                MessageController.build().setTracing(id, 1, cell_2_state == 2 || cell_2_state == 3 || cell_2_state == 4);
                ...
                if (cell_1_state == GnbStateRsp.gnbState.GNB_STATE_TRACE) {
                    MessageController.build().resetLastListState(deviceId, 0);
                }
                if (cell_2_state == GnbStateRsp.gnbState.GNB_STATE_TRACE) {
                    MessageController.build().resetLastListState(deviceId, 1);
                }
                state = new GnbStateRsp(cell_1_state, cell_2_state, gps_sync_state, time_sync_state, fair_sync_state,
                        sair_sync_state, sbIp.toString(), ssid, deviceId, bt_name);
                int vol = 0;
                idx += DWProtocol.OAM_STR_MAX;
                if ((idx + 3) < data.length) {
                    s = data[idx + 3] + data[idx + 2] + data[idx + 1] + data[idx];
     ...
     * 配置开启侦码反馈信息
     */
    public GnbTraceRsp gnbStartCatch(String source_data) {
        String[] data = source_data.split(",");
        GnbTraceRsp traceRsp = new GnbTraceRsp();
        String s = data[5] + data[4];
        int cmd_type = DataUtil.str2Int(s, 16);
        if (cmd_type == DWProtocol.gNB_2_UI_REPORT_UE_INFO) {// 定位103
            StringBuilder imsi = new StringBuilder();
            int idx = 8;
            if (idx < data.length) {
                String a = data[idx + 3] + data[idx + 2] + data[idx + 1] + data[idx];
                int cellId = DataUtil.str2Int(a, 16);
            ...
            SdkLog.D("gnbStartCatch(): imsi = " + imsi);
            if (imsi.length() == 15) {
                traceRsp.addImsi(imsi.toString());
            }
        } else if (cmd_type == DWProtocol.UI_2_gNB_OAM_MSG) { // 心跳标志
     ...
     * 结束侦码反馈信息
     */
    public GnbCmdRsp gnbStopCatch(String source_data) {
        return gnbCmdAck(source_data);
    }
    ...
    public GnbTraceRsp gnbStartTrace(String id, String source_data) {
        ...
        if (msg_type == DWProtocol.gNB_2_UI_REPORT_UE_INFO || msg_type == DWProtocol.gNB_2_UI_REPORT_LTE_UE_INFO) {// 定位103
            ...
            //String traceImsi = MessageController.build().getImsi(id,cellId);
            //SdkLog.D("gnbStartTrace(): imsi = " + imsi.toString() + ", traceImsi = " + traceImsi);
            if (curImsi.length() == 15) {
//                if (traceImsi != null && traceImsi.equals(curImsi)) {
...
//                }
    ...
    public GnbCmdRsp gnbStopTrace(String id,String source_data) {
        if (gnbCmdAck(source_data).getCmdType() ==DWProtocol.UI_2_gNB_STOP_TRACE||
                gnbCmdAck(source_data).getCmdType() ==DWProtocol.UI_2_gNB_STOP_LTE_TRACE) {
            if (gnbCmdAck(source_data).getRspValue() == DWProtocol.OAM_ACK_OK){
                MessageController.build().setTracing(id,gnbCmdAck(source_data).getCellId(),false);
    ...
    public GnbTraceRsp gnbStartControl(String id,String source_data) {
        ...
        String s = data[5] + data[4];
        int cmd = DataUtil.str2Int(s, 16);
        if (cmd == DWProtocol.gNB_2_UI_REPORT_UE_INFO) {// 定位103
            int idx = 8;
            if (idx < data.length) {
                String a = data[idx + 3] + data[idx + 2] + data[idx + 1] + data[idx];
                int cellId = DataUtil.str2Int(a, 16);
            ...
            //SdkLog.D("gnbStartTrace(): imsi = " + imsi.toString());
            if (imsi.length() == 15) {
                traceRsp.addImsi(imsi.toString());
                idx += DWProtocol.MAX_IMSI_LEN * 2;
                if (idx >= data.length) {
        ...
        } else if (cmd == DWProtocol.UI_2_gNB_OAM_MSG) { // 心跳标志
            GnbCmdRsp state = gnbCmdAck(source_data);
    ...
    public GnbFreqScanRsp gnbFreqScanRsp(String source_data) {
        SdkLog.I("gnbFreqScanRsp = " + source_data);
                        ...
                        if (idx + 32 < data.length){
                            ...
                            for (int i = idx; i < idx + 32; i++) {
                                if (data[i].equals("00")) {
                                    ...
                                    b_dev_sn[ret] = Byte.valueOf(data[i], 16);
                                    ret++;
                                }
                            }
        ...
        for (String uartdatum : uartdata) {
            ...
            if (data.length > 12) {
                ...
                if (msg_type == DBProtocol.MsgType.GR_MSG_SCAN_REPORT) {
                    ...
                    scanRsp.setBestPosition(best_position);
                    scanRsp.setMinNoisePower(min_noise_power);
...
//                    tmp = data[idx + 3] + data[idx + 2] + data[idx + 1] + data[idx];
//                    int signal_power = DataUtil.str2Int(tmp, 16);
                    tmp = data[idx + 3] + data[idx + 2] ;
                    int rsrp_second = DataUtil.str2Int(tmp, 16);
                    tmp = data[idx + 1] + data[idx] ;
                    int rsrp_first = DataUtil.str2Int(tmp, 16);
                    idx += 4;
                    if (idx >= data.length) {
                        break;
                    }
                    ...
                    scanRsp.setLockState(lock_state);
                    scanRsp.setRsrp_first(rsrp_first);
                    scanRsp.setRsrp_second(rsrp_second);
                    scanRsp.setScanResult(result);
                    scanRsp.setBestPosition(best_position);
                    scanRsp.setMinNoisePower(min_noise_power);
