...
public class BarChartUtil {
    ...
    private void initBarChart() {
        ...
        mBarChart.setDrawBorders(true);
         ...
         mBarChart.setBorderColor(Util.getColor(R.color.bar_chart_color)); //设置 chart 边框线的颜色。
        ...
        legend.setTextColor(Util.getColor(R.color.bar_chart_color));
    ...
    public void addBarDataSet(String name, int color){
        this.name = name;
        this.color = color;
        this.textColor = Util.getColor(R.color.bar_chart_color);

        BarDataSet set = createLineDataSet();
...
//        mBarChart.invalidate();
     ...
     * @param index
     */
    public void removeLineDataSet(int index) {
        mBarData.removeDataSet(index);
        //mLineDataSetArrayList.remove(index);
    }

    private BarDataSet createLineDataSet() {
        ...
        barDataSet.setValueTextColor(textColor);    //数据的颜色
        return barDataSet;
    }
    ...
    public void addEntry(int index, float number) {
        ...
        // 默认一分钟上号 60 个数据，保存十分钟数据，大于5分钟则间隔一分钟删除 60 个数据
        if (mBarData.getDataSets().get(0).getEntryCount() > 360) {
            AppLog.I("删除 60 个数据");
            for (int i = 0; i < 60; i++) {
                mBarData.getDataSets().get(0).removeFirst();
            }
            /*for (int i = 0; i < mLineData.getDataSets().get(0).getEntryCount(); i++){
                mLineData.getDataSets().get(0).getEntryForIndex(i).setX(i+1);
            }*/
            IBarDataSet iBarDataSet = mBarData.getDataSets().get(0);
            // 删除第一条线
            mBarData.removeDataSet(0);
            // 重新添加一条线
            mBarData.addDataSet(createLineDataSet());
            for (int i = 0; i < iBarDataSet.getEntryCount(); i++) {
                Entry entryForIndex = iBarDataSet.getEntryForIndex(i);
                entryForIndex.setX(i+1);
                mBarData.addEntry(entryForIndex, 0);
            }
        ...
        int count = mBarData.getEntryCount();
        count += 1;
        BarEntry entry = new BarEntry(count, number);
...
//        mBarData.groupBars(0f, groupSpace, barSpace);
        ...
        mBarChart.setVisibleXRangeMaximum(15);
        //移到某个位置
        if (isMove) mBarChart.moveViewToX(count);
    ...
    public void setXYAxis(float max, float min, int labelCount) {
        ...
        leftAxis.setTextColor(Util.getColor(R.color.bar_chart_color));
    ...
    public void setBorderColor(int color){
        mBarChart.setBorderColor(color); //设置 chart 边框线的颜色。
    }
//    //设置数据的颜色
