...
public class HistoryAdapter extends BaseAdapter {
    ...
    private List<JSONObject> jsonObjectListyg;   //通道一的列表
    private List<JSONObject> jsonObjectListld;
    private List<String> timeList;

    private Dialog mDialog;

    public HistoryAdapter(Context context, List<JSONObject> list1, List<JSONObject> list2, List<String> timeList) {
        this.mContext = context;
        this.jsonObjectListyg = list1;
        this.jsonObjectListld = list2;
        this.timeList = timeList;
    }

    public void setList(List<JSONObject> list1, List<JSONObject> list2, List<String> timeList) {
        this.jsonObjectListyg = list1;
        this.jsonObjectListld = list2;
        this.timeList = timeList;
    ...
    public View getView(int position, View convertView, ViewGroup viewGroup) {
        ...
        if (convertView == null) {
            ...
            convertView = LayoutInflater.from(mContext).inflate(R.layout.dialog_history_item_3g758cx, null);
        ...
        JSONObject cityList1 = jsonObjectListyg.get(position);
        JSONObject cityList2 = jsonObjectListld.get(position);
        String time = timeList.get(position);
        try {
            holder.tv_time.setText("记录" + (position + 1) + ": " + time);
            holder.tv_imsi1.setText("IMSI：" + cityList1.getString("imsi"));
            holder.tv_arfcnList1.setText("频点/pci：" + cityList1.getString("arfcnPciJsonArray").replaceAll("\\\\", ""));
            holder.tv_imsi2.setText("IMSI：" + cityList2.getString("imsi"));
            holder.tv_arfcnList2.setText("频点/pci：" + cityList2.getString("arfcnPciJsonArray").replaceAll("\\\\", ""));
