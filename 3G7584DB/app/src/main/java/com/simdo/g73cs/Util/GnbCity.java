...
public class GnbCity {
    ...
    public void init() {
        ...
        if (!list.contains("/")) {
            cityList.add(new CityBean(MainActivity.getInstance().getString(R.string.sz), "3000000", "9030000", "9030000", "9038000", "0"));
        } else {
    ...
    public boolean addCity(CityBean city) {
        ...
        if (add){
            if (city.getOffset_5g().isEmpty())
                city.setOffset_5g("3000000");
            if (city.getOffset_b34().isEmpty())
                city.setOffset_b34("9030000");
            if (city.getOffset_b39().isEmpty())
                city.setOffset_b39("9030000");
            if (city.getOffset_b40().isEmpty())
                city.setOffset_b40("9038000");
            if (city.getOffset_b41().isEmpty())
                city.setOffset_b41("0");
            cityList.add(city);
        }
        save();
        return add;
    ...
    public CityBean getCityBean(int pos) {
        if (cityList != null && cityList.size() > pos + 1) {
            return cityList.get(pos);
        }
        return null;
    }
