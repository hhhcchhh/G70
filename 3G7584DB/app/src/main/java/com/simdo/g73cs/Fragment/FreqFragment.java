    ...
    private void initView(View root) {
        ...
        if (MainActivity.ifDebug) {
            ...
            root.findViewById(R.id.tv_test).setOnClickListener(view -> {
                ...
                scanArfcnBeanList.add(new ScanArfcnBean("0", "0", 504990,
                        504990, 301, -67, 0, 0, 0, 0, 0, 0, 0, 0));
                scanArfcnBeanList.add(new ScanArfcnBean("0", "0", 504990,
                        504990, 302, -68, 0, 0, 0, 0, 0, 0, 0, 0));
        ...
        root.findViewById(R.id.tv_import_to_trace).setOnClickListener(view -> {
            ...
            for (DeviceInfoBean bean : deviceList) {
                if (bean.getWorkState() == GnbBean.DW_State.CATCH) {
                    Util.showToast(Util.getString(R.string.catching_tip));
                    return;
                } else if (bean.getWorkState() == GnbBean.DW_State.TRACE) {
                    Util.showToast(Util.getString(R.string.traceing_tip));
                    return;
                } else if (bean.getWorkState() == GnbBean.DW_State.FREQ_SCAN) {
                    Util.showToast("扫频中，请先结束扫频");
                    return;
                } else if (bean.getWorkState() == GnbBean.DW_State.NONE) noneCount++;
            }
            ...
            View view1 = LayoutInflater.from(mContext).inflate(R.layout.dialog_stop_trace, null);
            final TextView title = view1.findViewById(R.id.title);
            title.setText("确定导入数据至定位？");
            final TextView btn_ok = view1.findViewById(R.id.btn_ok);
            btn_ok.setOnClickListener(v -> {
                //清空原有的配置
                for (Fragment fragment : MainActivity.getInstance().getFragmentListBottom()){
                    if (fragment instanceof TraceCatchFragment ){
                        TraceCatchFragment mTraceCatchFragment = (TraceCatchFragment) fragment;
                       CfgTraceChildFragment mCfgTraceChildFragment =mTraceCatchFragment.getmCfgTraceChildFragment();
                        if (mCfgTraceChildFragment != null){
                            mCfgTraceChildFragment.clearCfg();
                        }
                    };
                }

                ArrayList<Integer> arfcnList = new ArrayList<>();
                for (ScanArfcnBean bean : scanArfcnBeanList) {
                    ...
                    if (arfcnList.contains(arfcn)) continue;
                    else {  // 不重复的才导入
                        MainActivity.getInstance().importArfcn(bean, pci);
                        arfcnList.add(arfcn);
                    }
                }
//            LinkedList<ScanArfcnBean> ll_scanArfcnBeanList = new LinkedList<>(scanArfcnBeanList);
//            LinkedList<LinkedList<ArfcnPciBean>> ll = FreqUtil.build().decFreqList_3g758cx(ll_scanArfcnBeanList);
//            AppLog.D("ll = " + ll.toString());

                Util.showToast(Util.getString(R.string.import_arfcn_success));
                MainActivity.getInstance().closeCustomDialog();
            });
            view1.findViewById(R.id.btn_cancel).setOnClickListener(v -> MainActivity.getInstance().closeCustomDialog());
            MainActivity.getInstance().showCustomDialog(view1, false);
    ...
    public void readArfcnData() {
        ...
        if (PaCtl.build().is3G758 && check_box_list.isEmpty()) {
            check_box_list.add(new CheckBoxBean("N41", 3, devB));
            ...
            check_box_list.add(new CheckBoxBean("B41", 4, devC));
            check_box_list.add(new CheckBoxBean("N1", 3, devA));
            check_box_list.add(new CheckBoxBean("N28", 1, devB));
            check_box_list.add(new CheckBoxBean("N79", 1, devA));
            check_box_list.add(new CheckBoxBean("B1", 2, devA));
            check_box_list.add(new CheckBoxBean("B5", 3, devC));
            check_box_list.add(new CheckBoxBean("B8", 3, devC));
            check_box_list.add(new CheckBoxBean("B34", 4, devC));
...
//    }
    ...
    public void dialogCfgFreq() {
        createCustomDialog();
        AlertDialog.Builder builder = new AlertDialog.Builder(mContext, R.style.Theme_G73CS_dialog);
        ...
        dialogCfgFreqBinding.ivCfgArfcn.setOnClickListener(view -> {
            /*if (!isStopScan){
    ...
    private void showCfgArfcnDialog() {
        ...
        CfgArfcnDialog mCfgArfcnDialog = new CfgArfcnDialog(requireActivity(), check_box_list);
            ...
            for (int i = 0; i < MainActivity.getInstance().getDeviceList().size(); i++) {
                if (MainActivity.getInstance().getDeviceList().get(i).getWorkState() == GnbBean.DW_State.FREQ_SCAN) {
                    ...
                    new Handler().postDelayed(() -> {
                        ...
                        else PaCtl.build().closePAById(id);
                    }, 300);
                }
            }
            isStopScan = true;
            progressDialog = ProgressDialog.show(requireActivity(), Util.getString(R.string.wait),
...
//                            MainActivity.getInstance().updateSteps(type, StepBean.State.success, Util.getString(R.string.freq_stoped));
                        ...
                        setFreqWorkState(MainActivity.getInstance().getDeviceList().get(i).getRsp().getDeviceId(), false); // 避免命令下发不响应，这里也做一次清除状态
                    }
                }
            }, 8000);
    ...
    public void clickStartFreqScan() {
        ...
        for (DeviceInfoBean bean : deviceList) {
            for (int i = 0; i < 4; i++) {
                if (bean.getTraceUtil().getWorkState(i) == GnbBean.DW_State.CATCH) {
                    Util.showToast(Util.getString(R.string.catching_tip));
                    return;
                } else if (bean.getTraceUtil().getWorkState(i) == GnbBean.DW_State.TRACE) {
                    Util.showToast(Util.getString(R.string.traceing_tip));
                    return;
                } else if (bean.getTraceUtil().getWorkState(i) == GnbBean.DW_State.NONE) {
                    noneCount++;
                } else if (bean.getTraceUtil().getWorkState(i) != GnbBean.DW_State.IDLE) {
        ...
        isGpsScan = PrefUtil.build().getValue("sync_mode_dw", "Air").toString().equals("GPS");
...
//        scanArfcnBeanList.clear();
        ...
        Util.showToast("开启扫网");
        for (int i = 0; i < MainActivity.getInstance().getDeviceList().size(); i++) {
            if (MainActivity.getInstance().getDeviceList().get(i).getWorkState() == GnbBean.DW_State.IDLE) {
                String id = MainActivity.getInstance().getDeviceList().get(i).getRsp().getDeviceId();
...
//                        MainActivity.getInstance().getDeviceList().get(i).getTraceUtil().setWorkState(bean.getChan_num() - 1, GnbBean.DW_State.FREQ_SCAN);
                        setFreqWorkState(id, true);
    ...
    int indexC = 1;//当前扫频到第几个

    private void freqScan(String id, String devName) {
        AppLog.I("FreqFragment freqScan() id" + id + " devName" + devName);
    ...
    public void startFreqScan(String id, CheckBoxBean bean, final int report_level) {
        ...
        new Handler().postDelayed(new Runnable() {
            @Override
            public void run() {
                PaCtl.build().openFreqPAByBand(id, bean.getText());
            }
        }, 100);
        int offset = 0;
        switch (bean.getText()) {
        ...
        new Handler().postDelayed(() -> {
            //refreshWorkState(getIndexById(id), GnbBean.DW_State.FREQ_SCAN, "扫频中");
            int async_enable = isGpsScan ? 0 : 1;
            AppLog.I("FreqFragment startFreqScan async_enable = " + async_enable + ", " + PaBean.build().toString());
            //输入此频段的频点列表，开始扫频
            MessageController.build().startDWFreqScan(id, report_level, isGpsScan ? 0 : 1, arfcn_list.size(), chan_id, arfcn_list, time_offset);
        }, 500);
    }
    ...
    public void onFreqScanRsp(String id, GnbFreqScanRsp rsp) {
        ...
        if (rsp != null) {
            AppLog.I("FreqFragment onFreqScanRsp() id " + id + "isStopScan" + isStopScan + "rsp" + rsp.toString());
            if (!isStopScan) {
                ...
                if (rsp.getReportStep() == 2) {
                    for (int i = 0; i < MainActivity.getInstance().getDeviceList().size(); i++) {
                        if (MainActivity.getInstance().getDeviceList().get(i).getWorkState() == GnbBean.DW_State.FREQ_SCAN) {
                            ...
                            MessageController.build().stopDWFreqScan(deviceId);
                            int finalI = i;
                            new Handler().postDelayed(() -> {
                                if (MainActivity.getInstance().getDeviceList().get(finalI).getRsp().getDevName().contains(devA))
                                    PaCtl.build().closePAById(deviceId);
                                else PaCtl.build().closePAById(deviceId);
                            }, 300);
                        }
                    }
                }
            ...
            //添加到列表中
            if (rsp.getScanResult() == DWProtocol.OAM_ACK_OK && rsp.getReportStep() == 1 && scanArfcnBeanList != null) {
...
//                MainActivity.getInstance().getDeviceList().get(indexById).setWorkState(GnbBean.DW_State.TRACE);
//                int type = MainActivity.getInstance().getDeviceList().get(indexById).getRsp().getDevName().contains(devA) ? 0 : 1;
//                MainActivity.getInstance().updateSteps(type, StepBean.State.success, Util.getString(R.string.freq_stoped));
                setFreqWorkState(id, false);
//                if (progressDialog != null) {
//                    progressDialog.dismiss();
//                    progressDialog = null;
//                }
    ...
    public void setFreqWorkState(String id, boolean isStart) {
        int logicIndex = DeviceUtil.getLogicIndexById(id);
        if (isStart) {
            if (isStopScan) {
                isStopScan = false;
                if (tv_start_scan != null) {
                ...
                //根据选择的频段改变相应的通道状态信息
                MainActivity.getInstance().updateLeftTvState(0, 100, -1, -1, scanning, false);
            ...
            MainActivity.getInstance().updateLeftTvState(0, 100, -1, -1, idle, false);
