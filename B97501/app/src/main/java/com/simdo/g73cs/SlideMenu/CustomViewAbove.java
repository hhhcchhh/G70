...
import com.simdo.g73cs.SlideMenu.SlidingMenu.OnClosedListener;
import com.simdo.g73cs.SlideMenu.SlidingMenu.OnOpenedListener;
...
public class CustomViewAbove extends ViewGroup {
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			if (thisTouchAllowed(ev)) {
				mIsBeingDragged = false;
				mIsUnableToDrag = false;
				if (isMenuOpen() && /*mViewBehind.menuTouchInQuickReturn(mContent, mCurItem, ev.getX() + mScrollX)*/diff < 9) {
					mQuickReturn = true;
					downX = 0;
				}
			} else {
	...
	public void scrollTo(int x, int y) {
		super.scrollTo(x, y);
		mScrollX = x;
		if (mViewBehind == null || mContent == null) return;
