...
public class GpsOffsetTestDialog extends Dialog {
    ...
    private void initView(View view) {
		...
		btn_ok.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View view) {
				...
				if (deviceList.get(0).getWorkState() != GnbBean.State.IDLE) {
					MainActivity.getInstance().showToast(mContext.getString(R.string.dev_busy_tip));
					return;
				}
				if (deviceList.get(0).getRsp().getGpsSyncState() != GnbStateRsp.Gps.SUCC) {
					MainActivity.getInstance().showToast(mContext.getString(R.string.gps_unlock));
					return;
				}
				deviceId = deviceList.get(0).getRsp().getDeviceId();
	...
	private void startTdRun(ArfcnBean bean){
		...
		PaCtl.build().openPA(deviceId, String.valueOf(arfcn));
		new Handler().postDelayed(new Runnable() {
			...
			public void run() {
				...
				MessageController.build().startTdMeasure(deviceId, cell_id, 0, arfcn, 0, 0);
			}
		}, 200);
	}
