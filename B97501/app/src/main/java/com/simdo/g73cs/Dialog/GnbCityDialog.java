...
public class GnbCityDialog extends Dialog {
        ...
        rb_mode.setOnCheckedChangeListener(new MyRadioGroup.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(MyRadioGroup group, int checkedId) {
                if (rb_mode.getCheckedRadioButtonId() == R.id.rb_air) {
                    //group_list.setVisibility(View.GONE);
                    PrefUtil.build().putValue("sync_mode", "空口");
                } else {
                    //group_list.setVisibility(View.VISIBLE);
                    PrefUtil.build().putValue("sync_mode", "GPS");
                    if (isShow){
                        isShow = false;
                        MainActivity.getInstance().showRemindDialog("强烈警告", "请确认当前城市时偏是否对应当前城市，时偏不正确可能导致不上号！");
                    }
                }
            }
        });
        cityList = GnbCity.build().getCityList();
        ListView mListView = view.findViewById(R.id.city_list);
    ...
    private void showAddCityDialog(int pos) {
        ...
        final EditText ed_city = view.findViewById(R.id.ed_city);

        final EditText ed_arfcn_5g = view.findViewById(R.id.ed_arfcn_5g);
        ...
        final EditText ed_arfcn_b34 = view.findViewById(R.id.ed_arfcn_b34);
        final EditText ed_arfcn_b39 = view.findViewById(R.id.ed_arfcn_b39);
        final EditText ed_arfcn_b40 = view.findViewById(R.id.ed_arfcn_b40);
        final EditText ed_arfcn_b41 = view.findViewById(R.id.ed_arfcn_b41);
        ...
        view.findViewById(R.id.btn_ok).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (city.length() == 0) {
                    MainActivity.getInstance().showToast(mContext.getResources().getString(R.string.error_city));
                    return;
                }
                ...
                if (time_offset_5g.length() == 0 || time_offset_b34.length() == 0 || time_offset_b39.length() == 0 || time_offset_b40.length() == 0 || time_offset_b41.length() == 0) {
                    MainActivity.getInstance().showToast(mContext.getResources().getString(R.string.error_empty_timingOffset));
                    return;
                }
				...
				if (offset_5g > 3000000 || offset_5g < 0) {
                    MainActivity.getInstance().showToast("5G" + mContext.getResources().getString(R.string.error_timingOffset));
                    return;
                }
				if (offset_b34 > 10000000 || offset_b34 < 0) {
					MainActivity.getInstance().showToast("B34" + mContext.getResources().getString(R.string.error_timingOffset));
					return;
				}
				if (offset_b39 > 10000000 || offset_b39 < 0) {
					MainActivity.getInstance().showToast("B39" + mContext.getResources().getString(R.string.error_timingOffset));
					return;
				}

				if (offset_b40 > 10000000 || offset_b40 < 0) {
					MainActivity.getInstance().showToast("B40" + mContext.getResources().getString(R.string.error_timingOffset));
					return;
				}

				if (offset_b41 > 10000000 || offset_b41 < 0) {
					MainActivity.getInstance().showToast("B41" + mContext.getResources().getString(R.string.error_timingOffset));
					return;
				}
                ...
                if (!GnbCity.build().addCity(c)) {
                    MainActivity.getInstance().showToast(mContext.getResources().getString(R.string.error_same_city));
                }
    ...
    private void showCitySelectDialog(final int position) {
        ...
        TextView tv_set_as_cur_city = view.findViewById(R.id.tv_set_as_cur_city);
        TextView tv_edit_city = view.findViewById(R.id.tv_edit_city);
        TextView tv_delete_city = view.findViewById(R.id.tv_delete_city);
