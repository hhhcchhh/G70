...
public class ConfigWifiDialog extends Dialog implements OnClickListener {
    ...
    public ConfigWifiDialog(Context context, LayoutInflater inflater, String deviceId, List<DeviceInfoBean> deviceList) {
        ...
        switch (v.getId()) {
            case R.id.btn_ok:
                ...
                for (DeviceInfoBean bean: deviceList){
                    MessageController.build().setDBWifiInfo(bean.getId(), ssid, psw);
                }
                dismiss();
                break;
