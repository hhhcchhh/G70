...
import com.simdo.dw_db_s.Ui.Adapter.FragmentAdapter;
import com.simdo.dw_db_s.Bean.EventMessageBean;
import com.simdo.dw_db_s.Ui.Fragment.DBFragment;
import com.simdo.dw_db_s.Ui.Fragment.DWFragment;
import com.simdo.dw_db_s.Ui.Fragment.SettingFragment;
import com.simdo.dw_db_s.Util.AppLog;
import com.simdo.dw_db_s.Util.CustomDialogUtil;
import com.simdo.dw_db_s.Util.GnbCity;
import com.simdo.dw_db_s.Util.PermissionsUtil;
import com.simdo.dw_db_s.Util.StatusBarUtil;
...
public class MainActivity extends AppCompatActivity implements SocketStateListener, FtpListener,
    ...
    public void onEvent(EventMessageBean emb) {
        switch (emb.getMsg()) {
            ...
            case "dismissProgressDialog" -> mSettingFragment.dismissProgressDialog(emb.getWhat());
        }
    }

    @Subscribe(threadMode = ThreadMode.BACKGROUND)
    public void onEventBackground(EventMessageBean emb) {
        switch (emb.getMsg()) {
...
//        GnbCity.build().
    ...
    public void onFtpPutFileRsp(String id, boolean state) {
        ...
        if (state) {
            ...
            emb.setMsg("dismissProgressDialog");
            emb.setWhat(-1);
    ...
    public void OnScpUpgradeFileRsp(boolean state) {
        ...
        if (state) {
            ...
            MessageController.build().setDBUpgrade(mDBFragment.getDeviceId(), 3, mDBFragment.getUpgradeFileName(), mDBFragment.getUpgradeFilePath());
        } else {
            mDWFragment.getDwDeviceList().get(0).setWorkState(GnbBean.DW_State.IDLE);
            mDWFragment.refreshStateView(getStr(R.string.state_prepared));
            showRemindDialog(getStr(R.string.bs_upgrade), getStr(R.string.state_upgrade_copying_file_fail));
