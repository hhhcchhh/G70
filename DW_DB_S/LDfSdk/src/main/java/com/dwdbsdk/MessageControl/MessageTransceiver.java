...
public class MessageTransceiver {
    ...
    private void dispatchDW(final String id, int msgType, int cmdType, final String data) {
        if (msgType == DWProtocol.gNB_2_UI_REPORT_UE_INFO || msgType == DWProtocol.gNB_2_UI_REPORT_LTE_UE_INFO) {
            switch (MessageController.build().getTraceType(id)) {
                ...
                case DWProtocol.TraceType.CATCH:
                    handler.post(new Runnable() {
                        @Override
                        public void run() {
                            messageObserver.onDWStartCatchRsp(id, MessageHelper.build().gnbStartCatch(id, data));
                        }
                    });
                    break;
        ...
        } else if (msgType == DWProtocol.UI_2_gNB_OAM_MSG){
            ...
            switch (cmdType) {
                ...
                case DWProtocol.UI_2_eNB_START_CONTROL:
                    handler.post(new Runnable() {
                        @Override
                        public void run() {
                            messageObserver.onDWStartControlRsp(id, MessageHelper.build().gnbStartControl(id, data));
                ...
                case DWProtocol.UI_2_eNB_START_CATCH:
                    handler.post(new Runnable() {
                        @Override
                        public void run() {
                            messageObserver.onDWStartCatchRsp(id, MessageHelper.build().gnbStartCatch(id, data));
                ...
                case DWProtocol.UI_2_eNB_STOP_CATCH:
                    handler.post(new Runnable() {
                        @Override
                        public void run() {
                            messageObserver.onDWStopCatchRsp(id, MessageHelper.build().gnbStopCatch(id, data));
                        }
                    });
                    break;
...
//                    break;
                ...
                case DWProtocol.OAM_MSG_RW_USER_DATA:
                    handler.post(new Runnable() {
                        @Override
                        public void run() {
                            GnbUserDataRsp userDataRsp = MessageHelper.build().gnbGetUserDataRsp(data);
                            if (userDataRsp.getRW() == 0) {
                                messageObserver.onDWGetUserData(id, userDataRsp);
                            } else {
                                messageObserver.onDWSetUserData(id, userDataRsp);
                            }
                        }
                    });
                    break;
                case DWProtocol.OAM_MSG_CFG_PA_TRX:
                    ...
                    break;
                case DWProtocol.OAM_MSG_PA_RW_CMD:
                    handler.post(new Runnable() {
                        @Override
                        public void run() {
                            messageObserver.onDWSetDataTo485(id,MessageHelper.build().gnbSetDataTo485Rsp(data));
                        }
                    });
                    break;
                case DWProtocol.OAM_MSG_SET_DATA_FWD:
                    handler.post(new Runnable() {
                        @Override
                        public void run() {
                            messageObserver.onDWSetDataFwd(id,MessageHelper.build().gnbReadDataFwd(data));
                        }
                    });
                    break;
                case DWProtocol.OAM_MSG_GET_CELL_CFG:
                    handler.post(new Runnable() {
                        @Override
                        public void run() {
                            messageObserver.onDWGetCellCfg(id,MessageHelper.build().gnbGetCellCfgRsp(id, data));
                        }
                    });
                    break;
                case DWProtocol.UI_2_gNB_REDIRECT_UE:
                case DWProtocol.UI_2_eNB_REDIRECT_UE:
                    handler.post(new Runnable() {
                        @Override
                        public void run() {
                            messageObserver.onDWRedirectUeCfg(id,MessageHelper.build().gnbRedirectUeRsp(id, data));
                        }
                    });
                    break;
                case DWProtocol.UI_2_gNB_SET_PLMN:
                case DWProtocol.UI_2_eNB_SET_PLMN:
                    handler.post(new Runnable() {
                        @Override
                        public void run() {
                            messageObserver.onDWResetPlmnCfg(id, MessageHelper.build().gnbResetPlmnRsp(id, data));
                        }
                    });
                    break;
                case DWProtocol.OAM_MSG_SET_REF_PWR:
                    handler.post(new Runnable() {
                        @Override
                        public void run() {
                            messageObserver.onDWSetPerPwrCfg(id, MessageHelper.build().gnbSetPerPwrRsp(id, data));
                        }
                    });
                    break;
                default:
            ...
            SdkLog.D("dispatch error id " + id + "[" + msgType + ", " + cmdType + "] " + data);
        }
    }

    private void dispatchDB(final String id, int msg_type, final String msg) {
        if (msg_type != DBProtocol.MsgType.GR_MSG_POWER_REPORT) SdkLog.D("dispatch DB data, id = " + id + ", msg_type: " + msg_type + ", msg = " + msg);
        switch (msg_type) {
            ...
            case DBProtocol.MsgType.GR_MSG_SENSE_REPORT:
                ...
                break;
            case DBProtocol.MsgType.GR_MSG_START_SS_SCAN:
                handler.post(new Runnable() {
                    @Override
                    public void run() {
                        messageObserver.onDBStartSSDetectRsp(id, MessageHelper.build().startSSDetect(msg));
                    }
                });
                break;
        }
    ...
    public void handleMessage(String type, String id, String tmpStr) {
        if (type.equals("DB")) {
            if (!olaDBStr.isEmpty() && !tmpStr.startsWith("11,11,a5,5a")) {
                tmpStr = olaDBStr + tmpStr;
                olaDBStr = "";
            }
            int last = 0;
            String[] uartdata = tmpStr.split("33,33,a5,5a,");
            int length = uartdata.length;
            String lastStr = uartdata[length - 1];
            if (!lastStr.endsWith("33,33,a5,5a")){
                last = 1;
                olaDBStr = lastStr + ",";
            }else olaDBStr = "";
            for (int i = 0; i < length - last; i++){
                String[] data = uartdata[i].split(",");
                if (data.length > 14) {
                    int idx = 12;
                    String s = data[idx + 3] + data[idx + 2] + data[idx + 1] + data[idx];
                    dispatchDB(id, DataUtil.str2Int(s, 16), uartdata[i]);
                }
            }
        } else {
            /* 指令配置返回信息带CELL_ID
             ...
             * */
            if (!olaDWStr.isEmpty() && !tmpStr.startsWith("11,11,a5,5a") && !tmpStr.startsWith("40,00,00,00") && !tmpStr.startsWith("44,00,00,00")) {
                tmpStr = olaDWStr + tmpStr;
                olaDWStr = "";
            }
            ...
            int length = uartdata.length;
            String lastStr = uartdata[length - 1];
            if (!lastStr.endsWith("ff,66,00,00")){
                last = 1;
                olaDWStr = lastStr + ",";
            }else olaDWStr = "";
            for (int i = 0; i < length - last; i++){
                ...
                if (data.length > 12) {
                    ...
                    dispatchDW(id, msg_type, DataUtil.str2Int(s, 16), uartdata[i]);
                }
            }
        }
    }
