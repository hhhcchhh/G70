...
public class GnbStartTrace {
    private final int u16MsgLength = DWHeader.headLength + 8 + 2 * DWProtocol.MAX_TARGET_UE_NUM * DWProtocol.MAX_IMSI_LEN;/* 定义消息的长度 */
    private final byte[] sendMsg = new byte[u16MsgLength];
    private int mMsgIdx;

    public GnbStartTrace(DWHeader header, int cell_id, int target_num, String target_ue) {
        mMsgIdx = 0;
    ...
    public GnbStartTrace(DWHeader header, int cell_id, List<UeidBean> ueid, int report_phone_type) {
        mMsgIdx = 0;
        for (int i = 0; i < u16MsgLength; i++) {
            sendMsg[i] = 0;
        }
