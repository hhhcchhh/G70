package com.dwdbsdk.Logcat;

import android.util.Log;

import com.dwdbsdk.File.FileItem;
import com.dwdbsdk.File.FileProtocol;
import com.dwdbsdk.File.FileSizeUtil;
import com.dwdbsdk.File.FileUtil;
import com.dwdbsdk.File.ZipPass;
...
public class SdkLog {
	...
	private static final String TAG = "LDfSdk";
	...
	public static void createFile() {
		...
		mLogFilePath = FileUtil.build().createOrAppendFile(FileProtocol.DIR_LOG, fileName,"LDfSdk start");
		if (mLogFilePath == null) {
	...
	private static void deleteLogFiles() {
		...
		if (file.exists()) {
			...
			SdkLog.D( "deleteLogFiles()  file size[zip] =  " + files.length);
			if (files.length > 100) {
