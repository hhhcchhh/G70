...
public class FreqFragment extends Fragment {
    ...
    private boolean isAirScan = false;
...
//            progressDialog.show();
    ...
    public void clickStartFreqScan() {
        ...
        isAirScan = PrefUtil.build().getValue("sync_mode", "Air").toString().equals("Air");
...
//        }
    ...
    public void startFreqScan(String id, CheckBoxBean bean, final int report_level) {
        ...
        new Handler().postDelayed(() -> {
            ...
            int async_enable = isAirScan ? 1 : 0;
            AppLog.I("FreqFragment startFreqScan async_enable = " + async_enable + ", " + PaBean.build().toString());
            //输入此频段的频点列表，开始扫频
            MessageController.build().startFreqScan(id, report_level, isAirScan ? 1 : 0, arfcn_list.size(), chan_id, arfcn_list, time_offset);
        }, 500);
    }
    ...
    public void setFreqWorkState(boolean isStart) {
        if (isStart) {
            if (isStopScan) {
                ...
                MainActivity.getInstance().updateSteps(0, StepBean.State.success, Util.getString(R.string.start_freq));
