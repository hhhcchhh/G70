...
public class MainActivity extends AppCompatActivity implements OnSocketChangeListener,
    ...
    //黑匣子序号
    private static int opNum = (int) PrefUtil.build().getValue(DateUtil.formateTimeYMD(System.currentTimeMillis()), 0);
    public static String account = "";
    public static boolean ifWriteLogin = false;  //是否记录登录
    ...
    private Button btn_state_test, btn_15, btn_10;
...
//                AppLog.D("oldFocus = " + oldFocus + "newFocus=" + newFocus);
//            }
//        });
...
//        drawerLayout.closeDrawer(drawerView);
    ...
    private void startActivity() {
        ...
        initSDK(); // 先绑定服务
        HistoryDBUtil.initDataBase();  //初始化sqlite
        CatchImsiDBUtil.initDataBase();
        LoginDBUtil.initDataBase();
        OperatorLogDBUtil.initDataBase();
        // 弹出登录界面
...
//        });
        ...
        if (MainActivity.ifDebug) btn_15.setVisibility(View.VISIBLE);
        else btn_15.setVisibility(View.GONE);
        btn_15.setOnClickListener(view -> {
                    mHandler.sendEmptyMessage(66);
                    Util.showToast("模拟15号消息成功", Toast.LENGTH_LONG);
...
//                AppLog.D("onDrawerStateChanged状态改变" + newState);
    ...
    private TextView tv_sync_state_1, tv_sync_state_2, tv_sync_state_3, tv_sync_state_4, tv_temp, tv_vol, tv_vol_warn, tv_latitude, tv_longitude;
    private String sync_state_1, sync_state_2, sync_state_3, sync_state_4;
    Double vol = 0d, temp = 0d;
    Double longitude = 0d, latitude = 0d;   //经纬度
    ...
    protected void initDrawerLeft() {
        ...
        tv_latitude = findViewById(R.id.device_latitude);
        tv_longitude = findViewById(R.id.device_longitude);
...
//        AppLog.I("updateSteps type = " + type + ", state = " + state + ", info = " + info);
        ...
        if (!nrStepsList.isEmpty()) {
            if (nrStepsList.get(0).getState() != StepBean.State.fail) {
        ...
        if (mSettingFragment.getmStepAdapter() != null) mSettingFragment.updatemStepAdapter(0);

        ContentValues values = new ContentValues();
        values.put("ACCOUNT", MainActivity.account);
        values.put("OPERATION", info);
        values.put("TIME", System.currentTimeMillis());
        if (OperatorLogDBUtil.insertOperationInDB(values) == -1) {
            Util.showToast("操作日志插入失败");
        }
    }
...
//        if (heartBeatCount == 0) heartBeatCount = 1;
        ...
        if (isAdd && deviceList.size() < 1) { // 是否为新数据，且当前已连接的设备未超过2个
            ...
            if (sync_mode.equals("Air")) {
                ...
                MessageController.build().setGps(deviceInfoBean.getRsp().getDeviceId(), 1, 0, 0);
            } else if (sync_mode.equals("Beidou")) {
                MessageController.build().setGps(deviceInfoBean.getRsp().getDeviceId(), 2, 0, 0);
            }

            if (!ifWriteLogin && !MainActivity.account.isEmpty()) {  //登录的时候没打印且已登录才走
                new Handler().postDelayed(() -> {
        ...
        if (rsp.getDevName().contains(devA)) {
            ...
            String gps_not_sync = getString(R.string.gps_not_sync);
            String beidou_not_sync = getString(R.string.beidou_not_sync);
            if (sync_mode.equals("Air")) {
                ...
                sync_state_1 = rsp.getGpsSyncState() == GnbStateRsp.Gps.SUCC ? getString(R.string.gps_sync) : gps_not_sync;
                sync_state_2 = rsp.getGpsSyncState() == GnbStateRsp.Gps.SUCC ? getString(R.string.gps_sync) : gps_not_sync;
                sync_state_3 = rsp.getGpsSyncState() == GnbStateRsp.Gps.SUCC ? getString(R.string.gps_sync) : gps_not_sync;
                sync_state_4 = rsp.getGpsSyncState() == GnbStateRsp.Gps.SUCC ? getString(R.string.gps_sync) : gps_not_sync;
            } else {
                sync_state_1 = rsp.getGpsSyncState() == GnbStateRsp.Gps.SUCC ? getString(R.string.beidou_sync) : beidou_not_sync;
                sync_state_2 = rsp.getGpsSyncState() == GnbStateRsp.Gps.SUCC ? getString(R.string.beidou_sync) : beidou_not_sync;
                sync_state_3 = rsp.getGpsSyncState() == GnbStateRsp.Gps.SUCC ? getString(R.string.beidou_sync) : beidou_not_sync;
                sync_state_4 = rsp.getGpsSyncState() == GnbStateRsp.Gps.SUCC ? getString(R.string.beidou_sync) : beidou_not_sync;
            }
        ...
        //经纬度
        longitude = rsp.getLongitude();
        latitude = rsp.getLatitude();
        ...
        if (vol < 1) {
            ...
            if (tipType == 1 || tipType == 0) {
                if (isTipcount > 6) {
                    ...
                    if (mTraceCatchFragment.getmCfgTraceChildFragment().getTv_do_btn().getText().equals(getString(R.string.stop))) {
        ...
        for (int i = 0; i < deviceList.size(); i++) {
            ...
            if (GnbStateRsp.gnbState.GNB_STATE_WAIT_CFG == deviceList.get(index).getRsp().getFirstState() &&
                ...
                if (MainActivity.getInstance().getDeviceList().get(i).getTraceUtil()
                        .getWorkState(GnbProtocol.CellId.FIRST) == GnbBean.State.STOP &&
                        MainActivity.getInstance().getDeviceList().get(i).getTraceUtil()
                                .getWorkState(GnbProtocol.CellId.SECOND) == GnbBean.State.STOP &&
                        MainActivity.getInstance().getDeviceList().get(i).getTraceUtil()
                                .getWorkState(GnbProtocol.CellId.THIRD) == GnbBean.State.STOP &&
                        MainActivity.getInstance().getDeviceList().get(i).getTraceUtil()
                                .getWorkState(GnbProtocol.CellId.FOURTH) == GnbBean.State.STOP) {
        ...
        int time_count = mTraceCatchFragment != null ? mTraceCatchFragment.getTime_count() : 60;
        for (int i = 0; i < deviceList.size(); i++) { //断连恢复状态
            TraceUtil traceUtil = deviceList.get(i).getTraceUtil();
            if ((deviceList.get(i).getWorkState() == GnbBean.State.IDLE
                    && traceUtil.getWorkState(0) == GnbBean.State.IDLE
                    && traceUtil.getWorkState(1) == GnbBean.State.IDLE
                    && traceUtil.getWorkState(2) == GnbBean.State.IDLE
                    && traceUtil.getWorkState(3) == GnbBean.State.IDLE)
                    && (GnbStateRsp.gnbState.GNB_STATE_TRACE == rsp.getFirstState() ||
                    GnbStateRsp.gnbState.GNB_STATE_TRACE == rsp.getSecondState() ||
                    GnbStateRsp.gnbState.GNB_STATE_TRACE == rsp.getThirdState() ||
                    GnbStateRsp.gnbState.GNB_STATE_TRACE == rsp.getFourthState())
            ) { //手机状态为空闲但板子状态为定位
                AppLog.I("onHeartStateRsp 断连恢复状态-----------------------------------------------------------");
                ...
                if (GnbStateRsp.gnbState.GNB_STATE_TRACE == rsp.getFirstState()) {
                    cell_id = 0;
                    traceUtil.setWorkState(cell_id, GnbBean.State.TRACE);
                    if (!mHandler.hasMessages(11)) {
                        mHandler.sendEmptyMessageDelayed(11, time_count * 1000L);
                    }
                    if (mTraceCatchFragment != null) {
                        // 刷新处于工作状态
                        mTraceCatchFragment.freshDoWorkState(type, cell_id, traceUtil.getImsi(cell_id), traceUtil.getArfcn(cell_id), traceUtil.getPci(cell_id), traceUtil.getBandWidth(cell_id));
                        String s = getString(R.string.cell) + mTraceCatchFragment.getCellStr(cell_id) + getString(R.string.traceing);
    ...
    private void stopNoUpCell() {
        boolean[] cellUp = {mTraceCatchFragment.isStartCatchHandler1(), mTraceCatchFragment.isStartCatchHandler2(), mTraceCatchFragment.isStartCatchHandler3(), mTraceCatchFragment.isStartCatchHandler4()};
        DeviceInfoBean device = deviceList.get(0);
    ...
    @Override
    public void onGetCellCfg(String s, JSONObject jsonObject) {
    ...
    public void onSocketStateChange(String id, int lastState, int newState) {
        ...
        if (newState == ConnectProtocol.SOCKET.STATE_CONNECTING) {
            ...
            if (deviceList.get(indexById).getWorkState() == GnbBean.State.NONE) {
                ...
                deviceList.get(indexById).getTraceUtil().setWorkState(GnbProtocol.CellId.THIRD, GnbBean.State.IDLE);
                deviceList.get(indexById).getTraceUtil().setWorkState(GnbProtocol.CellId.FOURTH, GnbBean.State.IDLE);
            ...
            deviceList.get(indexById).setWorkState(GnbBean.State.NONE);
            deviceList.get(indexById).getTraceUtil().setWorkState(GnbProtocol.CellId.FIRST, GnbBean.State.NONE);
            deviceList.get(indexById).getTraceUtil().setWorkState(GnbProtocol.CellId.SECOND, GnbBean.State.NONE);
            deviceList.get(indexById).getTraceUtil().setWorkState(GnbProtocol.CellId.THIRD, GnbBean.State.NONE);
            deviceList.get(indexById).getTraceUtil().setWorkState(GnbProtocol.CellId.FOURTH, GnbBean.State.NONE);
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case 2:
                    ...
                    break;
                case 66:
                    GnbTraceRsp rsp = new GnbTraceRsp();
                    GnbCmdRsp cmdRsp = new GnbCmdRsp(66, 66, 66, 66);
//                    rsp.setCmdRsp(cmdRsp);    //没上号的15号消息走这里
                    rsp.setRsrp(666);
                    rsp.setCellId(0);
                    List<String> imsiList = new ArrayList<>();
//                    imsiList.add("66666666666666" + traceCount++);
                    StringBuilder imsi = new StringBuilder();
                    for (int i = 0; i < 15; i++) {
                        imsi.append((int) (Math.random() * 10));
                    }
                    imsiList.add("66666666666666" + "1");
                    imsiList.add(imsi.toString());
                    rsp.setImsiList(imsiList);
                    mTraceCatchFragment.onStartTraceRsp("666666", rsp);
                    mHandler.sendEmptyMessageDelayed(66, 100);
                    break;
                case 998:
                case 999:
...
//                    }
//                    mTraceCatchFragment.getmCfgTraceChildFragment().getTv_do_btn().setEnabled(true);
//                    if (isTrace) {
//                        getDeviceList().get(0).setWorkState(GnbBean.State.TRACE);
...
//                    break;
    ...
    public void showCustomDialog(View view, boolean match) {
        Window window;
        if (mInInDialog != null) {
...
//            StatusBarUtil.setLightStatusBar(window, true);
    ...
    public void showCustomDialog(View view, boolean full, boolean setHeight) {
        Window window;
        if (mInInDialog != null) {
    ...
    public void deleteDev(String id) {
        ...
        for (int i = deviceList.size() - 1; i >= 0; i--) {
            if (deviceList.get(i).getRsp().getDeviceId().equals(id)) {
                ...
                String not_ready = getString(R.string.not_ready);
                updateProgress(type, 0, 0, not_ready, false);
                updateProgress(type, 0, 1, not_ready, false);
                updateProgress(type, 0, 2, not_ready, false);
                updateProgress(type, 0, 3, not_ready, false);
...
//        check_box_list.get(10).addAllArfcnList(list);
    ...
    public void setIfDebug(boolean isDebug) {
        ...
        btn_15.setVisibility(isDebug ? View.VISIBLE : View.GONE);
        btn_10.setVisibility(isDebug ? View.VISIBLE : View.GONE);
    }

    public ArrayList<StepBean> getNrStepsList() {
        return nrStepsList;
    }
    ...
    public static void opNumPlus() {
        opNum++;
        PrefUtil.build().putValue(DateUtil.formateTimeYMD(System.currentTimeMillis()), opNum);
    }
