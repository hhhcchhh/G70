...
public class ImsiAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> implements Filterable {
    ...
    private final List<ImsiBean> dataList;  //当前的显示列表
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        ...
        } else if (tracePlmn.equals("46000") || tracePlmn.equals("46002") || tracePlmn.equals("46007") || tracePlmn.equals("46004") || tracePlmn.equals("46008")) {
            tracePlmn = "46000";
        ...
        } else if (tracePlmn.equals("46015")) {
            tracePlmn = "46015";
            resId = R.mipmap.sva_icon;
        }
        if (resId != -1) viewHolder.iv_nr_lte_band.setImageResource(resId);
        viewHolder.tv_time.setText(DateUtil.formateTimeYMDHMS(item.getLatestTime()));
        ...
        viewHolder.tv_imsi.setOnClickListener(view -> {
            if (mOnItemClickListener != null) mOnItemClickListener.onItemClick(pos, dataList.get(pos));
        });
    ...
    //看起来好像model等于up的时候就是按强度排序的，等于no的时候就是按时间排序的
...
//                notifyDataSetChanged();
    ...
    /**
     * @description 将存储的数据放入列表中
     * @param
     * @return
     * @author Administrator
     * @time 2024/5/31 14:37
    ...
    /**
     * @description 将dataList覆盖到originalDataList中
     * @param
     * @return
     * @author Administrator
     * @time 2024/5/31 14:35
     */
    ...
    public void removeBeanOnOriginalList(ImsiBean bean) {
        originalDataList.remove(bean);
    }

    //更新所有相同的imsi号的状态
    public void updateOriginDataOriginList(String imsi, int state) {
        AppLog.D("updateOriginDataOriginList() imsi:" + imsi + " state:" + state);
        for (ImsiBean item : originalDataList) {
            if (item.getImsi().equals(imsi)) {
                item.setState(state);
                AppLog.D("imsi:" + imsi + " state:" + state);
            }
        }
        notifyDataSetChanged();
    }
    ...
    public interface OnRecyclerItemClickListener {
        void onItemClick(int pos, ImsiBean bean);
    }
