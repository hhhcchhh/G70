...
public class HistoryDialog extends Dialog implements OnClickListener {
    ...
    private void initView() {
        try {
            JSONArray jsonArray = HistoryDBUtil.getTraceCfgToDB();
        ...
        switch (v.getId()) {
            case R.id.tv_delete:
                ...
                view.findViewById(R.id.btn_ok).setOnClickListener(view1 -> {
                    //删除全部历史记录
                    int result = HistoryDBUtil.deleteAllTraceCfgToDB();
    ...
    private void showCityConfirmDialog(final int position, final boolean delete) {
        ...
        view.findViewById(R.id.btn_ok).setOnClickListener(v -> {
            ...
            if (delete) {
                ...
                HistoryDBUtil.deleteTraceCfgToDB(position);
                Util.showToast("删除成功");
            } else {
                //导入定位
