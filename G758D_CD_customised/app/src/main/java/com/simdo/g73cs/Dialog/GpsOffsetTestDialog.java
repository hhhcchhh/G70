...
public class GpsOffsetTestDialog extends Dialog {
    ...
    private void startTest() {
        ...
        if (!arfcn_5g.isEmpty() && NrBand.earfcn2band(Integer.parseInt(arfcn_5g)) <= 0) {
            Util.showToast("5G频点输入错误！");
            return;
        } else if (!arfcn_b34.isEmpty() && LteBand.earfcn2band(Integer.parseInt(arfcn_b34)) != 34) {
            Util.showToast("B34频点输入错误！");
            return;
        } else if (!arfcn_b39.isEmpty() && LteBand.earfcn2band(Integer.parseInt(arfcn_b39)) != 39) {
            Util.showToast("B39频点输入错误！");
            return;
        } else if (!arfcn_b40.isEmpty() && LteBand.earfcn2band(Integer.parseInt(arfcn_b40)) != 40) {
            Util.showToast("B40频点输入错误！");
            return;
        } else if (!arfcn_b41.isEmpty() && LteBand.earfcn2band(Integer.parseInt(arfcn_b41)) != 41) {
            Util.showToast("B41频点输入错误！");
            return;
        }
    ...
    private void startTdRun(ArfcnBean bean) {
        ...
        if (arfcn > 100000) {
            PaCtl.build().initPA(MainActivity.getInstance().getDeviceList().get(0).getRsp().getDeviceId(), String.valueOf(arfcn));
        } else {
            PaCtl.build().initLtePA(MainActivity.getInstance().getDeviceList().get(0).getRsp().getDeviceId(), String.valueOf(arfcn), cell_id);
        }
        int finalCell_id = cell_id;
        new Handler().postDelayed(() -> {
            // startTdMeasure(int cell_id, int swap_rf, int arfcn, int pk, int pa)
            MessageController.build().startTdMeasure(MainActivity.getInstance().getDeviceList().get(0).getRsp().getDeviceId(), finalCell_id, 0, arfcn, 0, 0);
        }, 200);
    }
