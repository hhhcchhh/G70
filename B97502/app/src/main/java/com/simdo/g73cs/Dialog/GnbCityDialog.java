            ...
            public void onCheckedChanged(MyRadioGroup group, int checkedId) {
                if (rb_mode.getCheckedRadioButtonId() == R.id.rb_air) {
                    ...
                    if (device == null) {
                        ...
                        if (sync_mode.equals("Air")) rb_mode.check(R.id.rb_air);
                        else if (sync_mode.equals("GPS")) rb_mode.check(R.id.rb_gps);
                        else rb_mode.check(R.id.rb_bei_dou);
                        return;
                    }
                    if (rb_mode.getCheckedRadioButtonId() == R.id.rb_gps){
                        PrefUtil.build().putValue("sync_mode", "GPS");
                        MessageController.build().setGps(0, 0, 0);
                    }else {
                        PrefUtil.build().putValue("sync_mode", "BeiDou");
                        MessageController.build().setGps(2, 0, 0);
                    }

                    if (isShow){
                        isShow = false;
                        MainActivity.getInstance().showRemindDialog(mContext.getString(R.string.warning_warning), mContext.getString(R.string.offset_warning_tip));
                    }
                }
            }
        });
