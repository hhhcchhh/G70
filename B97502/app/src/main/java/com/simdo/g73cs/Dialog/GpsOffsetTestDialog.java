...
public class GpsOffsetTestDialog extends Dialog {
	...
	private void startTdRun(ArfcnBean bean){
		...
		PaCtl.build().openPAByArfcn(device.getRsp().getDeviceId(), String.valueOf(arfcn));
		new Handler().postDelayed(new Runnable() {
			...
			public void run() {
				...
				MessageController.build().startTdMeasure(device.getRsp().getDeviceId(), cell_id, 0, arfcn, 0, 0);
			}
		}, 200);
	}
