...
public class FreqResultListAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder>{
   private LinkedList<ScanArfcnBean> dataList;
   private Context context;
   private ListItemListener listener;
   public FreqResultListAdapter(Context context, LinkedList<ScanArfcnBean> dataList, ListItemListener listener){
   ...
   public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int pos) {
      ...
      viewHolder.tv_dl_arfcn.setText(MessageFormat.format("{0}({1})", String.valueOf(item.getDl_arfcn()), item.getDl_arfcn() > 99999 ? NrBand.earfcn2band(item.getDl_arfcn()) : LteBand.earfcn2band(item.getDl_arfcn())));
